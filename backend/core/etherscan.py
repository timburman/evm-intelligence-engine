...
async def fetch_all_txs(address: str, chain_id: str = "1") -> dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        new_normal, last_block_n = await _sync_category(client, "txlist", address, chain_id, last_normal)
        if new_normal:
        ...
        new_internal, last_block_i = await _sync_category(client, "txlistinternal", address, chain_id, last_internal)
        if new_internal:
        ...
        new_erc20, last_block_e = await _sync_category(client, "tokentx", address, chain_id, last_erc20)
        if new_erc20:
...
async def _sync_category(client: httpx.AsyncClient, action: str, address: str, chain_id: str, last_block: int) -> tuple[list[dict], int]:
    """
